...
internal class EzPermissionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (notGranted.isNotEmpty()) {
            ActivityCompat.requestPermissions(
                this, getNotGrantedPermissions(), EzResult.CODE_PERMISSION
            )
        } else {
    ...
    private fun sendResult(
        granted: ArrayList<String>, denied: ArrayList<String>, permanentlyDenied: ArrayList<String>
    ) {
        val resultReceiver = intent.getParcelableExtra<ResultReceiver>(EzResult.EXTRA_RESULT_RECEIVER)
        resultReceiver?.send(
            EzResult.CODE_PERMISSION, bundleOf(
    ...
    private fun getGrantedPermissions() = getRequestedPermissions()?.filter {
        EzPermission.isGranted(
            this, it
        )
    }?.toTypedArray() ?: emptyArray()

    private fun getNotGrantedPermissions() = getRequestedPermissions()?.filter {
        !EzPermission.isGranted(
            this, it
        )
    }?.toTypedArray() ?: emptyArray()
