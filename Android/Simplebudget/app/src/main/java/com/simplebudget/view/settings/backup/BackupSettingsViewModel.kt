...
class BackupSettingsViewModel(
        ...
        if (authState is AuthState.Authenticated) {
            viewModelScope.launch {
                withContext(Dispatchers.IO) {
                    try {
                        if (appPreferences.getLastBackupDate() == null) {
                            getBackupDBMetaData(get(CloudStorage::class.java), auth)?.let {
                                appPreferences.saveLastBackupDate(it.lastUpdateDate)
                            }
                        }
                    } catch (e: Throwable) {
                        Log.e(
    ...
    fun onBackupActivated() {
        if (!appPreferences.isBackupEnabled()) {
            appPreferences.setBackupEnabled(true)
    ...
    fun onDeleteBackupConfirmationConfirmed() {
        viewModelScope.launch {
            ...
            try {
                withContext(Dispatchers.IO) {
                    deleteBackup(auth, get(CloudStorage::class.java))
                    appPreferences.saveLastBackupDate(null)
                }
            } catch (error: Throwable) {
